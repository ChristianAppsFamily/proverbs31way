...
import RoomLayout, { useRoomDate } from "../../components/layout/RoomLayout";
...
function ReactionButton({
  ...
  return (
    ...
    >
      ...
      <span>{label}</span>
      {count > 0 ? (
        <span className="text-xs ml-0.5" style={{ color: `${colors.text}99` }}>
          {count}
        </span>
      ) : null}
    </button>
...
function GardenCenter() {
  const { selectedDate } = useRoomDate();
  ...
  const content = useMemo(
    () => ({
      ...
      devotional: fb.devotional,
      reflectionPrompt: fb.reflectionPrompt,
    }),
    [fb],
  );
  ...
  return (
    <div className="space-y-8">
      <div>
        <p
          className="text-[22px] md:text-[26px] lg:text-[28px] leading-[1.55] italic"
      ...
      <div className="rounded-xl p-6" style={{ backgroundColor: colors.surface }}>
        <p
          ...
          style={{ fontFamily: '"Cormorant Garamond", Georgia, serif', color: colors.rose }}
        >
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <ReactionButton
          icon="🕊️"
          label="I needed this"
          count={reactions.needed}
          onClick={() => handleReaction("needed")}
        />
        <ReactionButton
          icon="🙏"
          label="Praying on this"
